...
grep -rn "console\.log\|console\.debug" src/ assets/ --include="*.js" --include="*.ts"
```
...
- HTML structure (`index.html`, templates)
- CSS/styling (`*.css`, theme system)
- Client-side JavaScript (`*.js` in assets/, components/)
...
test -f package.json && grep -q "@playwright/test" package.json

# If available, run browser tests
if [ -d tests ] && [ -n "$(ls tests/*.spec.js 2>/dev/null)" ]; then
  # Start local server
  ...
  if [ $TEST_RESULT -ne 0 ]; then
    echo "BROWSER TESTS FAILED - Phase verification cannot pass"
  fi
fi
```

**If Playwright is NOT installed but UI changes were made:**
- Flag as **HUMAN_NEEDED**: "Browser testing required but Playwright not configured"
- Include setup instructions in verification report
- DO NOT mark phase as PASSED without browser verification
...
Note: **Functional behavior CAN be verified with Playwright** (theme switching, search filtering, form submission). Don't flag functional tests as "human needed" — write automated tests.
...
| tests/search-filter.spec.js | ✗ FAILED | 1 test failed: empty state |

**Summary:** [Overall test health, or "No browser tests required for this phase"]
