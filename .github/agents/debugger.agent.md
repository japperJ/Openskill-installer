...
| **find_root_cause_only** | Find and document the root cause, don't fix | Steps 1-2 only (write and run reproducing test) for UI bugs |
...
If you discover a bug has already been fixed or claims to be verified:
...
- Write tests that confirm the fix works as expected

### Setting Up Playwright (if not installed)

```bash
# Install Playwright
npm init -y 2>/dev/null  # Create package.json if missing
...
test.describe('BUG-[timestamp]: [Bug Title]', () => {
  test('reproduces the bug', async ({ page }) => {
    // Navigate to the page
    await page.goto('http://localhost:8000');
    
    // Perform actions that trigger the bug
    await page.selectOption('#theme-toggle', 'light');
    
    // Assert the bug behavior (this should fail before fix)
    const htmlElement = page.locator('html');
    await expect(htmlElement).toHaveAttribute('data-color-mode', 'light');
  });
  
  test('verifies the fix', async ({ page }) => {
    await page.goto('http://localhost:8000');
    ...
    for (const theme of themes) {
      ...
      const htmlElement = page.locator('html');
      await expect(htmlElement).toHaveAttribute('data-color-mode', theme);
    }
  });
});
```
...
```bash
# Start local server first (required)
python -m http.server 8000 &  # Or: npx http-server
SERVER_PID=$!

# Run the test
npx playwright test tests/BUG-[timestamp]-*.spec.js

# Cleanup
kill $SERVER_PID
```

### Test Output in Debug File

Include test results in the Resolution section:
...
**Verification:** 
  - Code analysis: [What you checked]
  - Browser test: PASSED
    ```
    Running 2 tests using 1 worker
    ✓ BUG-20260213-143000: Theme selector › reproduces the bug (1.2s)
    ✓ BUG-20260213-143000: Theme selector › verifies the fix (1.5s)
    2 passed (3s)
    ```
...
await expect(page.locator('.card')).toHaveCSS('background-color', 'rgb(13, 17, 23)');
```
...
await expect(page.locator('#repos-count')).toHaveText('2');
```

**localStorage Testing:**
```javascript
await page.selectOption('#theme-toggle', 'dark');
...
```javascript
await expect(page.locator('#search-input')).toHaveAttribute('aria-label');
...
For bugs that DON'T involve UI/browser (backend, CLI, build scripts):
- Browser tests are OPTIONAL
- Standard verification techniques apply (unit tests, integration tests, manual testing)
...
6. **For UI/browser bugs:** Playwright tests have been run and passed

### UI/Browser Bugs - Browser Test is MANDATORY

For any bug involving visual/UI changes, user interactions, or client-side state:
- ❌ **Code inspection alone = NOT verified**
- ✅ **Code inspection + passing browser test = verified**
- Status 'verified' requires Playwright test output in Resolution section
- This applies even if the bug is already claimed to be fixed
...
**Commit:** [hash]
**Verification:** [How the fix was verified]
**Browser Test:** [PASSED/NOT_REQUIRED - include test output if UI bug]
