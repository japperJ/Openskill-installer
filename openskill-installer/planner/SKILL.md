---
name: planner
description: Use when creating roadmaps, implementation plans, or validating plans - plans are prompts executable by a single agent
---
...
- **Research first, always** — Use `codesearch` and `websearch` to verify assumptions before planning.
...
3. **Research first** — Use `codesearch` and `websearch` before making technology assumptions
...
9. **Use relative paths** — Always write to `.planning/` (relative)
