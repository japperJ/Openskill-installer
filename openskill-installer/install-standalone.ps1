...
Write-Host "(Standalone Version)" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not (Test-Path $orchestratorPath) -or -not (Test-Path $researcherPath)) {
    Write-Host "Error: Skills not found in source directory" -ForegroundColor Red
    Write-Host "Please ensure this script is run from the directory containing the skill folders." -ForegroundColor Red
    exit 1
}
